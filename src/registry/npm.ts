...
export async function discoverFromNPM(): Promise<RegistryServer[]> {
  ...
  for (const term of SEARCH_TERMS) {
    try {
      ...
      for (const obj of data.objects || []) {
        ...
        if (servers.has(pkg.name)) continue;

        // Check if it looks like an MCP server
        const nameLower = pkg.name.toLowerCase();
        const keywords = pkg.keywords || [];
        const hasMCP = nameLower.includes('mcp') ||
                      keywords.some((k: string) => k.toLowerCase().includes('mcp') || k.toLowerCase().includes('model-context-protocol'));
